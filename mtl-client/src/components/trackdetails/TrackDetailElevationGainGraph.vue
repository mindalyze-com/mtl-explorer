<template>
  <div class="tool-container">
    <h3 class="chart-title">Elevation gain</h3>
...
defineOptions({
  name: 'TrackDetailElevationGainGraph',
});
...
async function load(gpsTrackId: number) {
  ...
  const chartData = gpsTrackDetails.value.map(
    (item) => [item.pointTimestamp, item.elevationGainPerHourMovingWindow] as [Date | undefined, number | undefined]
  );
